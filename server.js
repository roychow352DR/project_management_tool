...
if(migrateTaskIds(state)){
 await writeFile(path.join(dir,'workspace.before-qa-ids.json'),beforeIdMigration,{flag:'wx'}).catch(error=>{if(error.code!=='EEXIST')throw error;});
...
http.createServer(async(req,res)=>{try{
 ...
 if(req.method==='PUT'){let body='';for await(const chunk of req){body+=chunk;if(body.length>1e6){res.writeHead(413);return res.end();}}
 let next;try{next=JSON.parse(body);if(!Array.isArray(next.projects)||!Array.isArray(next.tasks)||next.tasks.some(t=>!t.id||!t.title||!validTaskSchedule(t)||(t.backlog!==undefined&&typeof t.backlog!=='boolean')))throw Error();}catch{res.writeHead(400);return res.end('Invalid workspace');}
 migrateTaskIds(next);
 await (queue=queue.catch(()=>{}).then(async()=>{await writeFile(file+'.tmp',JSON.stringify(next,null,2));await rename(file+'.tmp',file);state=next;}));res.writeHead(204);return res.end();}
